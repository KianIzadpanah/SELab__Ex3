...
public class AccountBalanceCalculatorTest {
    ...
    void testTransactionHistoryAfterCalculation() {
        ...
        AccountBalanceCalculator.calculateBalance(testTransactions);
        List<Transaction> history = AccountBalanceCalculator.getTransactionHistory();

        assertEquals(testTransactions.size(), history.size(),
                "Transaction history should have the same number of transactions as the input list");

        assertTrue(history.containsAll(testTransactions),
